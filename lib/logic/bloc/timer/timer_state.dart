...
class TimerState extends Equatable {
  ...
  const TimerState({required this.stages, required this.currentStageIndex, required this.elapsed, required this.isRunning});

  factory TimerState.initial(List<WorkoutStage> stages) {
    return TimerState(stages: stages, currentStageIndex: 0, elapsed: Duration.zero, isRunning: false);
  }

  TimerState copyWith({List<WorkoutStage>? stages, int? currentStageIndex, Duration? elapsed, bool? isRunning}) {
    return TimerState(stages: stages ?? this.stages, currentStageIndex: currentStageIndex ?? this.currentStageIndex, elapsed: elapsed ?? this.elapsed, isRunning: isRunning ?? this.isRunning);
  }
